...
ap.add_argument("-f",'--filename',required=True,help="path to the directory containing all the files to predict")
...
#with torch.no_grad():
#if True:
listfile=os.listdir(args["filename"])
listfile=[p for p in listfile if ".csv" in p]
for o in listfile:
	df = pd.read_csv(args["filename"]+o)
	print(o,flush=True)
